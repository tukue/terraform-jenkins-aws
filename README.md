...
### Application Deployment Golden Path
...
| Developer experience | Before the golden path | With the golden path |
| :--- | :--- | :--- |
...
The generated workflow receives cluster and registry details from platform-managed GitHub Actions settings. `platform/app.env` only describes the application: source path, Dockerfile, port, health path, service exposure, and replica count. See [Kubernetes Application Golden Path](docs/kubernetes-application-golden-path.md) for setup and troubleshooting.
