...
resource "aws_iam_role_policy" "backstage_secrets" {
  name = "${var.environment}-backstage-secrets"
  role = aws_iam_role.backstage.id
    ...
    Statement = [
      {
        ...
        Resource = module.backstage_postgres.master_user_secret_arn
      },
      {
        Effect = "Allow"
        Action = [
        ...
        Resource = module.backstage_postgres.kms_key_arn
      }
    ]
  })
}
